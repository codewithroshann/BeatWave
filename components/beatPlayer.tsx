...
export default function BeatPlayer() {
  ...
  const beats: BeatPlayer = useSelector(
    (state: any) => state.musicPlayer as BeatPlayer
  );
  ...
  return (
    <div
      className={`flex flex-col w-full ${
        beat.playing == true ? "block" : "hidden"
      } supports-[backdrop-filter]:bg-background/60 duration-300 cursor-pointer bg-background/95 backdrop-blur-md text-white rounded-md overflow-hidden fixed bottom-0 z-50`}
    >
      {/* Progress bar at the top */}
      <div
        ref={progressBarRef}
        className="w-full bg-gray-700 cursor-pointer relative h-2"
      ...
      <div className="flex items-center justify-between w-full px-2 py-1">
        ...
        <div className="flex items-center gap-3">
          <div className="relative h-16 w-16 rounded-full overflow-hidden cursor-pointer music-cover">
            <Image
              ...
              fill
              className={`object-cover animate-${
                isPlaying == true ? "spin" : "none"
              }`}
            />
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-[10px] md:text-xs">
              {beat?.title ? `${beat.title.slice(0, 40)}...` : "Untitled"}
            </span>
        ...
        <div className="flex items-center gap-4 flex-1 justify-center mx-3">
          ...
          <button
            onClick={togglePlay}
            className="h-10 w-10 rounded-full border border-white bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
          >
            {isPlaying ? (
